const buttonSize = (
  height: string,
  paddingInline: string,
  gap: string,
  iconSize: string,
  squareRadius: string,
  pressedRadius: string,
  innerRadius: string,
  outlineWidth: string,
) => ({
  height,
  "padding-inline": paddingInline,
  gap,
  "icon-size": iconSize,
  "square-radius": squareRadius,
  "pressed-radius": pressedRadius,
  "inner-radius": innerRadius,
  "outline-width": outlineWidth,
});

const sliderSize = (trackHeight: string, handleHeight: string, trackRadius: string) => ({
  "track-height": trackHeight,
  "handle-height": handleHeight,
  "track-radius": trackRadius,
});

const avatarSize = (size: string, iconSize: string, squareRadius: string) => ({
  size,
  "icon-size": iconSize,
  "square-radius": squareRadius,
});

const fabSize = (
  height: string,
  iconSize: string,
  radius: string,
  paddingInline: string,
  gap: string,
) => ({
  height,
  "icon-size": iconSize,
  radius,
  "padding-inline": paddingInline,
  gap,
});

export const componentTokens = {
  "focus-ring": {
    width: "3px",
    offset: "2px",
    color: "{color.secondary}",
  },
  "touch-target": {
    size: "48px",
  },
  density: {
    step: "4px",
  },
  icon: {
    "font-family": "'Material Symbols Outlined'",
    size: "24px",
    fill: "0",
    weight: "400",
    grade: "0",
    "optical-size": "24",
  },
  button: {
    xs: buttonSize(
      "32px",
      "{space.md}",
      "{space.xs}",
      "20px",
      "{radius.md}",
      "{radius.sm}",
      "{radius.xs}",
      "1px",
    ),
    sm: buttonSize(
      "40px",
      "{space.lg}",
      "{space.sm}",
      "20px",
      "{radius.md}",
      "{radius.sm}",
      "{radius.sm}",
      "1px",
    ),
    md: buttonSize(
      "56px",
      "{space.xl}",
      "{space.sm}",
      "24px",
      "{radius.lg}",
      "{radius.md}",
      "{radius.sm}",
      "1px",
    ),
    lg: buttonSize(
      "96px",
      "{space.3xl}",
      "{space.md}",
      "32px",
      "{radius.xl}",
      "{radius.lg}",
      "{radius.lg}",
      "2px",
    ),
    xl: buttonSize(
      "136px",
      "64px",
      "{space.lg}",
      "40px",
      "{radius.xl}",
      "{radius.lg}",
      "{radius.lg-increased}",
      "3px",
    ),
  },
  "button-group": {
    "connected-gap": "{space.2xs}",
    "xs-gap": "18px",
    "sm-gap": "{space.md}",
    "md-gap": "{space.sm}",
    "lg-gap": "{space.sm}",
    "xl-gap": "{space.sm}",
  },
  fab: {
    sm: fabSize("56px", "24px", "{radius.lg}", "{space.lg}", "{space.md}"),
    md: fabSize("80px", "28px", "{radius.lg-increased}", "26px", "{space.md}"),
    lg: fabSize("96px", "36px", "{radius.xl}", "28px", "{space.lg}"),
    elevation: "{elevation.level3}",
    "hovered-elevation": "{elevation.level4}",
  },
  "fab-menu": {
    "close-size": "56px",
    "item-height": "56px",
    "item-padding-inline": "{space.xl}",
    "item-gap": "{space.sm}",
    "item-icon-size": "24px",
    gap: "{space.xs}",
  },
  toolbar: {
    height: "64px",
    padding: "{space.sm}",
    gap: "{space.xs}",
    "floating-radius": "{radius.full}",
    "floating-elevation": "{elevation.level3}",
  },
  separator: {
    thickness: "1px",
    inset: "{space.lg}",
  },
  breadcrumbs: {
    gap: "{space.xs}",
    "separator-size": "18px",
  },
  tabs: {
    height: "48px",
    "icon-height": "64px",
    "padding-inline": "{space.lg}",
    "stacked-gap": "{space.2xs}",
    "inline-gap": "{space.sm}",
    "icon-size": "24px",
    "divider-width": "1px",
    "primary-indicator-thickness": "3px",
    "primary-indicator-min-length": "24px",
    "secondary-indicator-thickness": "2px",
    "scrollable-inset": "52px",
    "panel-shift": "30px",
  },
  disclosure: {
    height: "56px",
    "padding-inline": "{space.lg}",
    gap: "{space.lg}",
    "indicator-size": "24px",
    radius: "{radius.lg}",
    "segment-radius": "{radius.xs}",
    "outline-width": "1px",
    "group-gap": "{space.sm}",
    "segmented-gap": "{space.2xs}",
  },
  "navigation-item": {
    "indicator-width": "56px",
    "indicator-height": "32px",
    "horizontal-indicator-height": "40px",
    "icon-size": "24px",
    "label-gap": "{space.xs}",
    "horizontal-gap": "{space.xs}",
    "horizontal-padding-inline": "{space.lg}",
    "badge-size": "{badge.large-size}",
    "badge-dot-size": "{badge.small-size}",
  },
  "navigation-bar": {
    height: "64px",
    "horizontal-gap": "{space.sm}",
    "container-color": "{color.surface-container}",
  },
  "navigation-rail": {
    width: "96px",
    "expanded-width": "220px",
    "padding-block": "{space.2xl}",
    "item-gap": "{space.xs}",
    "item-height": "64px",
    "expanded-item-height": "56px",
    "expanded-padding-inline": "20px",
    "container-color": "{color.surface}",
  },
  "navigation-drawer": {
    width: "360px",
    padding: "{space.md}",
    radius: "{radius.lg}",
    "item-height": "56px",
    "item-gap": "{space.md}",
    "item-padding-inline-end": "{space.xl}",
    "headline-padding-inline": "{space.lg}",
    "container-color": "{color.surface-container-low}",
    elevation: "{elevation.level1}",
  },
  "app-bar": {
    "small-height": "64px",
    "medium-height": "112px",
    "large-height": "120px",
    "padding-inline": "{space.xs}",
    "title-padding-inline": "{space.lg}",
    "medium-padding-block-end": "{space.md}",
    "large-padding-block-end": "{space.md}",
    "scrolled-color": "{color.surface-container}",
  },
  "text-field": {
    height: "56px",
    "padding-inline": "{space.lg}",
    gap: "{space.md}",
    "icon-size": "24px",
    radius: "{radius.xs}",
    "container-color": "{color.surface-container-highest}",
    "indicator-width": "1px",
    "focus-indicator-width": "2px",
    "label-inset": "{space.sm}",
    "supporting-gap": "{space.xs}",
  },
  "search-field": {
    height: "56px",
    radius: "{radius.full}",
    "container-color": "{color.surface-container-high}",
  },
  checkbox: {
    size: "18px",
    radius: "2px",
    "outline-width": "2px",
    "state-layer-size": "40px",
  },
  radio: {
    size: "20px",
    "dot-size": "10px",
    "outline-width": "2px",
    "state-layer-size": "40px",
  },
  switch: {
    "track-width": "52px",
    "track-height": "32px",
    "outline-width": "2px",
    "handle-size": "16px",
    "selected-handle-size": "24px",
    "pressed-handle-size": "28px",
    "icon-size": "16px",
    "state-layer-size": "40px",
  },
  slider: {
    xs: sliderSize("16px", "44px", "{radius.sm}"),
    sm: sliderSize("24px", "44px", "{radius.sm}"),
    md: sliderSize("40px", "52px", "{radius.md}"),
    lg: sliderSize("56px", "68px", "{radius.lg}"),
    xl: sliderSize("96px", "108px", "{radius.xl}"),
    "handle-width": "4px",
    "handle-gap": "6px",
    "inner-radius": "2px",
    "stop-size": "4px",
    "value-indicator-height": "44px",
    "vertical-length": "200px",
  },
  progress: {
    "track-height": "4px",
    gap: "4px",
    "stop-size": "4px",
    "wave-amplitude": "3px",
    "wave-length": "40px",
    "circular-size": "40px",
    "circular-track-width": "4px",
    "loading-size": "48px",
    "loading-indicator-size": "38px",
  },
  "drop-zone": {
    padding: "{space.xl}",
    radius: "{radius.lg}",
    "outline-width": "1px",
  },
  list: {
    "item-height": "56px",
    "item-padding-inline": "{space.lg}",
    "item-gap": "{space.lg}",
    "item-icon-size": "24px",
    "segmented-gap": "{space.2xs}",
    "segmented-radius": "{radius.lg}",
    "segmented-item-radius": "{radius.xs}",
    "grid-min-column-width": "160px",
    "selection-size": "18px",
    "selection-radius": "2px",
    "selection-outline-width": "2px",
  },
  menu: {
    radius: "{radius.lg}",
    padding: "{space.xs}",
    elevation: "{elevation.level2}",
    "min-width": "112px",
    "max-width": "280px",
    "item-height": "48px",
    "item-padding-inline": "{space.md}",
    "item-gap": "{space.md}",
    "item-icon-size": "20px",
    "item-radius": "{radius.xs}",
    "item-edge-radius": "{radius.md}",
    "selected-item-radius": "{radius.md}",
  },
  table: {
    "header-height": "56px",
    "row-height": "52px",
    "cell-padding-inline": "{space.lg}",
    "cell-padding-inline-compact": "{space.md}",
    "divider-width": "1px",
    radius: "{radius.md}",
    "sort-icon-size": "18px",
    "resizer-width": "16px",
    "resizer-indicator-width": "2px",
    indent: "{space.xl}",
  },
  tree: {
    "item-height": "48px",
    "item-padding-inline": "{space.md}",
    "item-gap": "{space.sm}",
    "item-icon-size": "24px",
    "item-radius": "{radius.full}",
    indent: "{space.xl}",
  },
  dropdown: {
    width: "280px",
    height: "56px",
    "padding-inline": "{space.lg}",
    gap: "{space.md}",
    "icon-size": "24px",
    radius: "{radius.xs}",
    "container-color": "{color.surface-container-highest}",
    "indicator-width": "1px",
    "focus-indicator-width": "2px",
    "label-inset": "{space.sm}",
    "supporting-gap": "{space.xs}",
  },
  calendar: {
    padding: "{space.md}",
    "header-height": "48px",
    "nav-size": "40px",
    "icon-size": "24px",
    "cell-width": "48px",
    "cell-height": "48px",
    "day-size": "40px",
    "day-radius": "{radius.full}",
    "today-outline-width": "1px",
    "month-gap": "{space.xl}",
  },
  "date-picker": {
    "container-color": "{color.surface-container-high}",
    radius: "{radius.lg}",
    elevation: "{elevation.level3}",
    "segment-radius": "{radius.xs}",
    "trigger-size": "40px",
  },
  chip: {
    height: "32px",
    radius: "{radius.sm}",
    "padding-inline": "{space.lg}",
    "icon-padding-inline": "{space.sm}",
    gap: "{space.sm}",
    "icon-size": "18px",
    "avatar-size": "24px",
    "outline-width": "1px",
    elevation: "{elevation.level1}",
    "hovered-elevation": "{elevation.level2}",
    "set-gap": "{space.sm}",
  },
  card: {
    padding: "{space.lg}",
    radius: "{radius.md}",
    elevation: "{elevation.level1}",
    "elevated-container-color": "{color.surface-container-low}",
    "filled-container-color": "{color.surface-container-highest}",
    "outlined-container-color": "{color.surface}",
    "outline-color": "{color.outline-variant}",
    "outline-width": "1px",
  },
  badge: {
    "small-size": "6px",
    "large-size": "16px",
    "padding-inline": "{space.xs}",
  },
  avatar: {
    xs: avatarSize("24px", "16px", "{radius.xs}"),
    sm: avatarSize("32px", "20px", "{radius.sm}"),
    md: avatarSize("40px", "24px", "{radius.md}"),
    lg: avatarSize("56px", "32px", "{radius.lg}"),
    xl: avatarSize("88px", "48px", "{radius.xl}"),
  },
  keyboard: {
    "min-size": "24px",
    "padding-inline": "{space.xs}",
    radius: "{radius.xs}",
    "outline-width": "1px",
    depth: "2px",
  },
  skeleton: {
    color: "{color.surface-container-highest}",
    "highlight-color": "{color.surface-container-low}",
    "text-radius": "{radius.xs}",
    "rect-radius": "{radius.md}",
    "circle-size": "40px",
  },
  "empty-state": {
    "max-inline-size": "400px",
    padding: "{space.xl}",
    "expanded-padding": "{space.3xl}",
    gap: "{space.sm}",
    "icon-size": "40px",
    "icon-container-size": "88px",
    "expanded-icon-container-size": "120px",
    "icon-container-radius": "{radius.full}",
    "actions-gap": "{space.sm}",
  },
  "color-thumb": {
    size: "24px",
    "dragging-size": "32px",
    "border-width": "2px",
    "border-color": "{color.surface-container-lowest}",
    elevation: "{elevation.level1}",
  },
  "color-area": {
    size: "256px",
    radius: "{radius.lg}",
  },
  "color-slider": {
    "track-height": "24px",
    "track-radius": "{radius.full}",
    "vertical-length": "200px",
  },
  "color-swatch": {
    xs: { size: "24px" },
    sm: { size: "32px" },
    md: { size: "40px" },
    lg: { size: "48px" },
    xl: { size: "64px" },
    "square-radius": "{radius.sm}",
    "outline-color": "{color.outline-variant}",
    "outline-width": "1px",
    "selected-ring-width": "2px",
    "selected-ring-color": "{color.on-surface}",
    "selected-gap-color": "{color.surface-container-lowest}",
    "picker-gap": "{space.sm}",
  },
  "color-checker": {
    size: "16px",
    color: "{color.surface-container-highest}",
    background: "{color.surface-container-lowest}",
  },
  "close-button": {
    size: "40px",
    "icon-size": "24px",
  },
  dialog: {
    "container-color": "{color.surface-container-high}",
    radius: "{radius.xl}",
    elevation: "{elevation.level3}",
    padding: "{space.xl}",
    gap: "{space.lg}",
    "actions-gap": "{space.sm}",
    "min-width": "280px",
    "max-width": "560px",
    margin: "{space.xl}",
    "fullscreen-container-color": "{color.surface}",
    "fullscreen-header-height": "56px",
    "fullscreen-padding-inline": "{space.lg}",
    "scrim-opacity": "0.32",
  },
  popover: {
    "container-color": "{color.surface-container}",
    radius: "{radius.lg}",
    elevation: "{elevation.level2}",
    padding: "{space.lg}",
    "arrow-width": "12px",
    "arrow-height": "6px",
  },
  tooltip: {
    "plain-container-color": "{color.inverse-surface}",
    "plain-radius": "{radius.xs}",
    "plain-min-height": "24px",
    "plain-padding-inline": "{space.sm}",
    "plain-padding-block": "{space.xs}",
    "plain-max-width": "200px",
    "rich-container-color": "{color.surface-container}",
    "rich-radius": "{radius.md}",
    "rich-padding-inline": "{space.lg}",
    "rich-padding-block": "{space.md}",
    "rich-max-width": "320px",
    "rich-gap": "{space.xs}",
    "rich-elevation": "{elevation.level2}",
  },
  snackbar: {
    "container-color": "{color.inverse-surface}",
    "action-color": "{color.inverse-primary}",
    radius: "{radius.xs}",
    elevation: "{elevation.level3}",
    "min-height": "48px",
    "min-width": "344px",
    "max-width": "600px",
    "padding-inline-start": "{space.lg}",
    "padding-inline-end": "{space.sm}",
    "padding-block": "14px",
    gap: "{space.xs}",
    margin: "{space.lg}",
    "stack-gap": "{space.sm}",
  },
  "bottom-sheet": {
    "container-color": "{color.surface-container-low}",
    radius: "{radius.xl}",
    elevation: "{elevation.level1}",
    "max-width": "640px",
    "top-margin": "72px",
    padding: "{space.xl}",
    "handle-width": "32px",
    "handle-height": "4px",
    "handle-area-height": "{touch-target.size}",
  },
  "side-sheet": {
    "container-color": "{color.surface-container-low}",
    radius: "{radius.lg}",
    elevation: "{elevation.level1}",
    width: "400px",
    "min-margin": "56px",
    padding: "{space.xl}",
    "header-height": "72px",
    "header-padding-inline-end": "{space.md}",
  },
};
