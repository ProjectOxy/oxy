...
The decorator wraps every story in `OxyProvider` from `@oxy/ui`; `tools/storybook/src/seed-theme.ts` turns the `seed` global into a theme with `@material/material-color-utilities` and `createTheme` from `@oxy/tokens`.
