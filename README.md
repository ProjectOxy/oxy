...
- **Pages** (`.github/workflows/pages.yml`) publishes the static Storybook to GitHub Pages on every push to `main` and on `workflow_dispatch` (the build emits only relative URLs, so it works from the `/oxy/` subpath without a `base` override; the deploy job is skipped with a warning while Pages is disabled); the CI `verify` job additionally uploads the npm tarballs of every publishable package as the `npm-packages` artifact.
